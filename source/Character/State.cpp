...
void State::displayState()
{
    ...
    // Rectangle circle = Feet();
    DrawTextureRec(character->sprite.texture, frameRec, character->pos, WHITE);
    // DrawCircle(circle.x, circle.y, circle.width, {0, 255, 0, 230}); // Draw feet circle for debugging
}
...
// Rectangle State::Feet() const
// {
//     float width = fabs(frameRec.width);
//     float height = frameRec.height;
//     float radius = width / 2 - 1;
//     float centerX = character->pos.x + radius + 1;
//     float centerY = character->pos.y + height - radius + 2;
//     return Rectangle{
//         centerX,
//         centerY,
//         radius,
