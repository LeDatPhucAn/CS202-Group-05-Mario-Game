...
void InputBox::update()
{
    ...
    if (!head || head->isActivated)
    {
        ...
        inputHandler->update();
        if (inputHandler->isTexting() && IsKeyPressed(KEY_ENTER) && onClick)
            onClick();
    }
    if (next)
        next->update();
...
void InputBox::draw()
{
    ...
    if (!head || head->isActivated)
    {
        ...
        if (inputHandler->isTexting() && (inputHandler->getFramesCounter() / 20) % 2 == 0)
        {
            ...
            UI::drawtext2(inputHandler->getText(), rect.x + rect.width / 2, rect.y + rect.height / 2, TextColor);
...
{
    // if (scenehandler->getCurrentScene() == sceneType::MENU) {
    //     return;
    // }
